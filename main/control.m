...
for iit = 1:iitMax
    ...
    for jit = 1:jitMax
        ...
        % Reaction forces
        [vec_FR, vec_FG, K_avg, DV, vec_FE, vec_BG, F_avg, FJ, vec_FF] = III_reactions(S, T, velocityHit, mass, gravity, K, F, dLdxS, dLdyS, S_2, vec_O);
        % Final equations
        [TE_avg, TC_avg, heatFlux, heatTransfer, areaGround, cooling, TP_new, velocityEnd, FI, deltalKE, deltaKE, Y_avg, G_avg, landPoisson, landMaterialProp, deformation] = III_final(S_2, velocityHit, vec_FR, vec_FF, vec_FG, vec_FE, mass, T, Y, G, diameter, podMaterialProp, S, TE, TC, heatCapacity, TP);
        ...
        writematrix("              vec_VR = " + double(vec_mag(vec_FR)),'Test Output','WriteMode','append');
        writematrix("              vec_VF = " + double(vec_mag(vec_FF)),'Test Output','WriteMode','append');
        writematrix("              vec_VG = " + double(vec_mag(vec_FG)),'Test Output','WriteMode','append');
        writematrix("              vec_VE = " + double(vec_mag(vec_FE)),'Test Output','WriteMode','append');
