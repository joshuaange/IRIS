function [vec_VR, vec_VG, vec_VE, vec_VF, K_avg] = III_reactions(S, T, gravity, velocityHit, jit, S_2, K, s, iit, minimumRestitution)
%Reaction forces
%   Called by control.m
...
vec_VG = [S(1) S(2) S(3); 0 0 0];%-T*0.01*gravity];
...
if jit > 1
    vec_VE = [vec_VR(1,1)+vec_VR(2,1) vec_VR(1,2)+vec_VR(2,2) vec_VR(1,3)+vec_VR(2,3); -K_calc*(s{iit,jit-1}.velocityHit(2,1)+s{iit,jit-1}.vec_VR(2,1))*cos(vec_alpha(s{iit,jit-1}.vec_VR)) -K_calc*(s{iit,jit-1}.velocityHit(2,2)+s{iit,jit-1}.vec_VR(2,2))*cos(vec_beta(s{iit,jit-1}.vec_VR)) -K_calc*(s{iit,jit-1}.velocityHit(2,3)+s{iit,jit-1}.vec_VR(2,3))*cos(vec_gamma(s{iit,jit-1}.vec_VR))];
    %vec_VE = [vec_VR(1,1)+vec_VR(2,1) vec_VR(1,2)+vec_VR(2,2) vec_VR(1,3)+vec_VR(2,3); -K_calc*(velocityHit(2,1)+vec_VR(2,1))*cos(vec_alpha(vec_VRfake)) -K_calc*(velocityHit(2,2)+vec_VR(2,2))*cos(vec_beta(vec_VRfake)) -K_calc*(velocityHit(2,3)+vec_VR(2,3))*cos(vec_gamma(vec_VRfake))];
else
