...
if jit <= Kt_i
    ...
    A_n_ij = [S_ij(1),S_ij(2),S_ij(3); (-(mag(B_ij))/T)/Kt_i * cos(falpha(B_ij)),(-(mag(B_ij))/T)/Kt_i * cos(fbeta(B_ij)),(-(mag(B_ij))/T)/Kt_i * cos(fgamma(B_ij))];
end
...
A_r_ij = [S_ij(1),S_ij(2),S_ij(3); (d*2)*(mag(q_ij))*T*cos(falpha(VQ_ij)), (d*2)*(mag(q_ij))*T*cos(fbeta(VQ_ij)), (d*2)*(mag(q_ij))*T*cos(fgamma(VQ_ij))];
