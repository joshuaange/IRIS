...
display("...Rotation");
    % Influence
Wlength = (mag(O_ij))/(T*(d/2)); % Finds angular velocity from linear velocity
...
if ZROT == cos(pi/2)
    ...
    % Ground Friction Angular Velocity
p_ij = [S_ij(1),S_ij(2),S_ij(3);(Wlength*XROT), (Wlength*YROT), (Wlength*ZROT)];
    % New Angular Velocity
Q_ij = [R_ij(1),R_ij(2),R_ij(3); q_ij(2,1),q_ij(2,2),q_ij(2,3)];
...
% the ending angular velocity
    % Applying Friction
if Q_ij(2,1) > 0
    Q_ij(2,1) = Q_ij(2,1) +(((d/2)*(m*(A_fr_ij(2,1))))/I)*T;
elseif Q_ij(2,1) < 0
    Q_ij(2,1) = Q_ij(2,1) -(((d/2)*(m*(A_fr_ij(2,1))))/I)*T;
end
if Q_ij(2,2) > 0
    Q_ij(2,2) = Q_ij(2,2) +(((d/2)*(m*(A_fr_ij(2,2))))/I)*T;
elseif Q_ij(2,2) < 0
    Q_ij(2,2) = Q_ij(2,2) -(((d/2)*(m*(A_fr_ij(2,2))))/I)*T;
end
if Q_ij(2,3) > 0
    Q_ij(2,3) = Q_ij(2,3) +(((d/2)*(m*(A_fr_ij(2,3))))/I)*T;
elseif Q_ij(2,3) < 0
    Q_ij(2,3) = Q_ij(2,3) -(((d/2)*(m*(A_fr_ij(2,3))))/I)*T;
end
% Reactionary
