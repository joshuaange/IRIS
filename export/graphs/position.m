...
for iit = 1:i_max
    if isempty(b{iit}) == 0
        for iteration = 1:floor(double(b{iit}.t_f_i)/((t_stride+t_step)/2))
            ...
            plot(timeA+multitude,b{iit}.z_i(multitude)-L_min,'s','Color','red','HandleVisibility','off');
        end
        ...
        plot(timeA,b{iit}.C_i(3)-L_min,'s','Color','red','HandleVisibility','off');
    end
    for jit = 1:j_max
        if isempty(s{iit,jit}) == 0
            ...
            plot(timeA,s{iit,jit}.C_ij(3)-L_min,'s','Color','red','HandleVisibility','off');
            timeA = timeA + 0.5*T;
            ...
            plot(timeA,s{iit,jit}.Cn_ij(3)-L_min,'s','Color','red','HandleVisibility','off');
        end
    end
end
