...
view(3);
%legend;
% fplot3(x_i,y_i,z_i,[0 10],'DisplayName','Trajectory','lineWidth',1,'Color','red');
% X_i = X_sphere*(d/2)+x_i(t_p);
% Y_i = Y_sphere*(d/2)+y_i(t_p);
% Z_i = Z_sphere*(d/2)+z_i(t_p);
% surf(double(X_i), double(Y_i), double(Z_i),'DisplayName','Pod','facealpha',0.75,'edgealpha',0.5) surf(double(X_i), double(Y_i), double(Z_i),'DisplayName','Pod','facealpha',0.75,'edgealpha',0.5)
% fmesh(s{1,1}.NN_ij) % Mesh Plot
% quiver3(s{iit,jit}.P_ij(1,1),s{iit,jit}.P_ij(1,2),s{iit,jit}.P_ij(1,3),s{iit,jit}.P_ij(2,1),s{iit,jit}.P_ij(2,2),s{iit,jit}.P_ij(2,3),'lineWidth',3,'Color','red','MaxHeadSize',0.5,'DisplayName','Traced Path Segment');
% quiver3(s{iit,jit}.B_ij(1,1),s{iit,jit}.B_ij(1,2),s{iit,jit}.B_ij(1,3),s{iit,jit}.B_ij(2,1),s{iit,jit}.B_ij(2,2),s{iit,jit}.B_ij(2,3),'lineWidth',3,'Color','blue','MaxHeadSize',0.5,'DisplayName','Traced Path Segment');
%             plot3(double(s{iit,jit}.Rn_ij(1)),double(s{iit,jit}.Rn_ij(2)),double(s{iit,jit}.Rn_ij(3)),'Marker','s','DisplayName','Impact Point');
%             plot3(double(s{iit,jit}.R_ij(1)),double(s{iit,jit}.R_ij(2)),double(s{iit,jit}.R_ij(3)),'Marker','o','DisplayName','Impact Point');
...
%surfc(Y_l_x, Y_l_y, Y_l_z,'EdgeAlpha',0.1,'FaceAlpha',1,'DisplayName','Youngs Modulus') % Mesh Plot
 ...
 for iit = 1:i_max
     if isempty(b{iit}) == 0
        % Trajectory
        fplot3(b{iit}.x_i,b{iit}.y_i,b{iit}.z_i,[0 double(b{iit}.t_f_i)],'DisplayName','Trajectory','lineWidth',1,'Color','blue');
        % Velocities
     ...
     for jit = 1:j_max
         if isempty(s{iit,jit}) == 0
            % Traced Segment Path
            quiver3(s{iit,jit}.O_ij(1,1),s{iit,jit}.O_ij(1,2),s{iit,jit}.O_ij(1,3),s{iit,jit}.O_ij(2,1),s{iit,jit}.O_ij(2,2),s{iit,jit}.O_ij(2,3),'lineWidth',1,'Color','blue','MaxHeadSize',0.5,'DisplayName','Traced Path Segment');
